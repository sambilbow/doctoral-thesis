...
\textit{Designing Rich AR Experience} proposes the following solution:
\begin{enumerate}
    \item Centre the experience on two or more sensory interactions
    \item Invoke a meaningful relationship between the real and virtual
    \item Implement an AR subform from an enactivist perspective
\end{enumerate}

% \subsubsection{Sensory Engagement}
Whether it is Dewey's concept of the ``live creature'', or the contemporary enactivist's framing of the importance of embodiment, the AR experience ought to be \textit{centred on two or more sensory interactions}. It may include any combination of sensory interaction types, i.e visual (vision), auditory (hearing), vestibular (balance), olfactory (smell), gustatory (taste), and somatosensory (touch). This approach goes hand-in-hand with 
